...
int CQPrint::Dialog (char *PrinterName, int Pages)
{
	...
	if (PrinterName == NULL) {
		...
		// a printer is given...
		if (m_PrinterDC.CreateDC (NULL, CA2W(PrinterName), NULL, NULL) == 0) {
...
HPRIVATEFONT CQPrint::AddFontToEnvironment (char *FontName, int normalHeight, int bigHeight)
{
	...
	char charBuf[32];
	strcpy_s (charBuf,FontName);
	mbstowcs(log.lfFaceName, charBuf, 32);
...
void CQPrint::Print (HPRIVATEFONT font, CString strText, UINT Format, int left)
{
	...
	if (strText == "") {
		GetTextExtentPoint32 (m_DC.GetSafeHdc(), L"A", 1, &Size);
	} else  {
...
void CQPrint::Line (int Mode)
{
	...
	m_DC.MoveTo (CALCY(m_Margins.left), m_y);
	m_DC.LineTo (m_DimDraw.cx - CALCY(m_Margins.right), m_y);
...
// return: -1 = error, otherwise 0
//         in the case of an error you can use GetErrorCode() to get the error-code
int CQPrint::InsertBitmapFromView (int fmt, CSize *dest, int left)
{
	...
	CSize		sizeSource, sizeDest;
	int			lx,
				Width = m_DimDraw.cx - CALCY(m_Margins.left) - CALCY(m_Margins.right);
    CRect       margins;
	...
	// if dest == NULL --> bitmap has to print in original-size
	if (dest == NULL) {
		float scale = Width/sizeSource.cx;
		sizeDest.cx = (Width);
		sizeDest.cy = (sizeSource.cy*scale);
	} else {
		// in sizeDest we save the desired printing-size
		sizeDest.cx = CALCY(dest->cx);
		sizeDest.cy = CALCY(dest->cy);
